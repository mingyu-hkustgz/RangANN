...
int main(int argc, char *argv[]) {
    ...
    sprintf(result_path, "./results/%s_hnsw_filter.log",dataset);
    sprintf(index_path, "./DATA/%s/%s_naive.hnsw", dataset, dataset);
    Matrix<float> X(data_path);
    Matrix<float> Q(query_path);
    IndexFilter filter_hnsw(X.n, X.d);
    filter_hnsw.load_index(index_path);
    ...
    for (int i = 0; i < Q.n; i++) {
        ...
        ans1 = filter_hnsw.naive_range_search(SeQ, K, efSearch);
