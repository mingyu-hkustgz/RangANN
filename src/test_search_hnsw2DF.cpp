    ...
    while (iarg != -1) {
        ...
        switch (iarg) {
            ...
            case 'e':
                ...
                break;
            case 'f':
                if (optarg) fan = atoi(optarg);
                break;
        }
    }
    sprintf(query_path, "%s%s_query.fvecs", source, dataset);
    sprintf(data_path, "%s%s_base.fvecs", source, dataset);
    sprintf(result_path, "./results@%d/%s/%s_HBI2D%d_%d.log", K, dataset, dataset, fan, length_bound);
    sprintf(index_path, "./DATA/%s/%s_2D%d.hnsw", dataset, dataset, fan);
    Matrix<float> X(data_path);
    Matrix<float> Q(query_path);
    hnswlib::HierarchicalNSWStatic<float>::static_base_data_ = (char *) X.data;
    Index2DF hnsw2D(X.n, X.d, fan);
