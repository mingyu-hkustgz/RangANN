...
for data in "${datasets[@]}"; do
  ...
  start_time=$(date +%s)
  ./cmake-build-debug/src/test_build_hnsw1D -d ${data} -s "${store_path}/${data}/" -i "./DATA/${data}/"
  end_time=$(date +%s)
  ...
  start_time=$(date +%s)
  ./cmake-build-debug/src/test_build_hnsw2D -d ${data} -s "${store_path}/${data}/" -i "./DATA/${data}/"
  end_time=$(date +%s)
  duration=$((end_time - start_time))
