//
// Created by Mingyu on 24-8-5.
//

#ifndef DEEPBIT_MATRIX_H
#define DEEPBIT_MATRIX_H
...
Matrix<T> resize_matrix(const Matrix<T> &A, uint32_t _n, uint32_t _d) {
    ...
    std::cerr << "Resize Matrix with Shape - " << _n << " " << _d << std::endl;
    Matrix<T> result(_n, _d);
    for (int i = 0; i < _n; i++)
        for (int j = 0; j < _d; j++)
            result.data[i * _d + j] = A.data[i * A.d + j];
    return result;
}
